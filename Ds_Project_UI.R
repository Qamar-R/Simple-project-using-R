  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("color_plots", "Choose Color for Plots", 
                  choices = c("red", "green", "blue", "yellow", "black","thistle1","firebrick4","gold")),
      width = 3
    ),
...
server <- function(input, output, session) {
  ...
  output$histogram <- renderPlot({
    ...
    if (!is.null(x_var) && !is.null(data())) {
      if (x_var %in% c("no_of_trainings", "age", "previous_year_rating", "length_of_service", "avg_training_score")) {
        ggplot(data(), aes_string(x = x_var)) +
          geom_histogram(binwidth = 1, fill = input$color_plots, color = "black") +
  ...
  # K-means clustering
  observeEvent(input$tab == "K-means", {
    if (!is.null(data())) {
  ...
  output$kmeans_plot <- renderPlot({
    if (!is.null(kmeans_result())) {
      ggplot(data(), aes(x = input$x_axis, y = input$y_axis, color = as.factor(kmeans_result()$cluster))) +
        geom_point() +
